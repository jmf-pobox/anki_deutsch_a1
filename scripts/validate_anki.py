...
def validate_test_cards() -> Tuple[bool, int, int]:
    ...
    test_cards = [
        ...
        # Test cards with field substitution
        {
            ...
            "fields": {"Word": "Haus", "Meaning": "house", "Explanation": "Noun meaning"},
    ...
    for i, case in enumerate(problematic_cases, 1):
        ...
        if is_valid == should_be_valid:
            if should_be_valid:
                ...
                print(f"      Issues found: {len(sum(issues.values(), []))}")
        else:
            print(f"   ❌ INCORRECT - Expected valid={should_be_valid}, got valid={is_valid}")
            all_correct = False
            if not is_valid:
                total_issues += len(sum(issues.values(), []))
                for category, category_issues in issues.items():
                    for issue in category_issues:
                        print(f"      • {category}: {issue}")
