...
class AnkiValidator:
    ...
    @staticmethod
    def validate_cloze_card(content: str, fields: Dict[str, str]) -> Tuple[bool, List[str]]:
        ...
        if not cloze_matches:
            issues.append("No valid cloze deletions found (expected {{c1::text}} format)")
        else:
            ...
            if len(unique_numbers) > 1:
                if max(unique_numbers) - min(unique_numbers) + 1 != len(unique_numbers):
    ...
    @staticmethod
    def validate_field_references(template: str, fields: Dict[str, str]) -> Tuple[bool, List[str]]:
        ...
        for field_name in field_matches:
            if field_name not in fields:
                issues.append(f"Template references undefined field: {{{{ {field_name} }}}}")
        
        # Check for empty required fields that would cause blank content
        critical_fields = ["Text", "Word", "German", "English"]  # Common critical fields
        for field_name in critical_fields:
            if field_name in fields and not fields[field_name].strip():
                if f"{{{{{field_name}}}}}" in template:
                    issues.append(f"Critical field '{field_name}' is empty but referenced in template")
        
        return (len(issues) == 0, issues)
    
    @staticmethod
    def validate_media_paths(card_content: str, media_dir: Path | None = None) -> Tuple[bool, List[str]]:
        """Validates media file references are accessible.
        
        Args:
            card_content: Content with media references like [sound:file.mp3] or <img src="...">
    ...
    def validate_cloze_syntax(content: str) -> Tuple[bool, List[str]]:
        ...
        if not valid_clozes and '{{c' in content:
            issues.append("Found cloze-like patterns but none match valid {{c1::text}} format")
