...
class DebugDeckGenerator:
    ...
    def create_debug_deck(issue_type: str) -> Path:
        ...
        if issue_type not in issue_generators:
            ...
            raise ValueError(
                f"Unknown issue type '{issue_type}'. Available: {available_types}"
            )
    ...
    def generate_test_cards(pattern: str, count: int = 5) -> list[dict[str, Any]]:
        ...
        if pattern not in patterns:
            ...
            raise ValueError(
                f"Unknown pattern '{pattern}'. Available: {available_patterns}"
            )
    ...
    def add_diagnostic_fields(card_data: dict[str, Any]) -> dict[str, Any]:
        ...
        enhanced_data["Debug_Expected_Behavior"] = card_data.get(
            "expected_behavior", "See notes"
        )
        enhanced_data["Debug_Test_Notes"] = card_data.get(
            "test_notes", "Generated for debugging"
        )
    ...
    def _generate_multi_cloze_test() -> list[dict[str, Any]]:
        ...
        return [
            ...
            {
                "Text": (
                    "{{c1::{{Article}}}} {{c2::{{Noun}}}} ist {{c3::{{Adjective}}}}"
                ),
                "Article": "Das",
                "Noun": "Haus",
                "Adjective": "groß",
                ...
                "debug_issue_type": "multi_cloze",
                "expected_behavior": (
                    "Should substitute fields and create multiple cards"
                ),
    ...
    def _create_multi_cloze_pattern(index: int) -> dict[str, Any]:
        ...
        patterns = [
            ...
            (
                "{{c1::Die}} {{c2::Frau}} ist {{c3::schön}}",
                "Triple cloze with feminine",
            ),
