...
import genanki  # type: ignore[import-untyped]  # External dependency boundary - no stubs available
...
class AnkiDeckGenerator:
    ...
    def _get_audio(self, text: str) -> bytes | None:
        ...
        try:
            response = self.audio_service.synthesize_speech(  # type: ignore[attr-defined]  # Legacy code boundary - deprecated method
                Text=text,
                OutputFormat="mp3",
                VoiceId="Vicki",
                Engine="neural",
            ...
            if "AudioStream" in response:
                return response["AudioStream"].read()  # type: ignore[no-any-return]  # boto3 boundary - returns Any from untyped library
...
def create_deck(name: str, description: str) -> "Deck":
    """Create a new Anki deck."""
    deck_id = random.randrange(1 << 30, 1 << 31)
    return genanki.Deck(deck_id, name, description)


def add_note(deck: "Deck", fields: list[str], model: "Model") -> None:
    """Add a note to the deck."""
    note = genanki.Note(model=model, fields=fields)
    deck.add_note(note)


def write_to_file(deck: "Deck", file: str, timestamp: float | None = None) -> None:
