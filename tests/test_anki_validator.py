"""Tests for AnkiValidator - comprehensive validation of Anki application compatibility."""
...
class TestAnkiValidator:
    ...
    def test_validate_card_complete_multiple_issues(self) -> None:
        ...
        content = "{{UndefinedField}} {{c0::{{Word}}}}"  # Missing field + invalid cloze numbering
    ...
    @pytest.mark.parametrize("cloze_content,expected_valid", [
        ("{{c1::Der}} Mann", True),
        ("{{c1::}} empty", False),
        ("{{c1::Der}} {{c2::Mann}}", True),
    ...
    def test_cloze_syntax_patterns(self, cloze_content: str, expected_valid: bool) -> None:
